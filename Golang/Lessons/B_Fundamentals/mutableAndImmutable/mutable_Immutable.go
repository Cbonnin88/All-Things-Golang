...
func main() {
	...
	var slice []float32 = []float32{19.7, 12.8, 29.4}
	y1 := slice
	y1[0] = 124.5
	fmt.Println(slice, y1)
	// Maps are also mutable datatype:
	var x map[string]int = map[string]int{"Hello There": 4}
	y3 := x
	x["7"] = 7
	y3["y"] = 50
	fmt.Println(x, y3)
	// Using a mutable array:
	var arr [3]string = [3]string{"cow", "egg", "french"}
	yArr := arr
	yArr[0] = "Zombie"
	fmt.Println(arr, yArr)

	// The slice we create for our function below:
	var slice1 []float32 = []float32{123.66, 77.45, 23.49}
